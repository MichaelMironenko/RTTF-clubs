Vue.component("input-field", {
  props: {
    ...
    required: Boolean,
    errorMessage: String,
    sectionKey: String, // Уникальный ключ для раздела данных
    inputKey: String, // Уникальный ключ для инпута
  },
  computed: {
    inputId() {
      return `${this.sectionKey}-${this.inputKey}`;
    },
    charsLeft() {
  ...
  methods: {
    updateValue(newValue) {
      this.$emit("input", newValue); // Эмиттируем событие в родительский компонент
    },
  },
  watch: {
    value(newValue) {
  ...
  template: `
    <div class="input-wrapper">
      ...
      <input v-if="type === 'text'" :type="type" :id="id" :class="{ error: error }" :maxlength="maxlength" v-model="value" @input="updateValue">
      <textarea v-else :id="id" :class="{ error: error }" :maxlength="maxlength" :rows="rows" v-model="value" @input="updateValue"></textarea>
      <div class="input-feedback">
        <span class="error-message" v-if="error">{{ errorMessage }}</span>
        <span class="character-count" v-if="charsLeft <= 10">{{ charsLeft }}</span>
      </div>
    </div>`,
});
...
Vue.component("photo-upload", {
  ...
  <img src="delete.svg" @click="removeImage(index)" class="delete-button" />
     ...
     <div v-if="singlePhoto && imageData" class="single-image">
  ...
  methods: {
    ...
    handleImageUpload(data) {
      if (this.sections && data.sectionId in this.sections) {
        ...
        // Не вызываем сохранение здесь, а только обновляем данные
      } else {
    ...
    addEvent() {
      this.newEvent.day = this.currentDay;
      this.sections.activities.events.push({
        ...this.newEvent,
        day: this.currentDay,
      });
      this.newEvent = {
    ...
    saveEvent(index) {
      // Сброс индекса редактирования для возврата к режиму просмотра
      console.log("saveEvent:", index, this.sections.activities.events[index]);
